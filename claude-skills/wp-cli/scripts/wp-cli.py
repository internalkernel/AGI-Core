...
def get_data_dir() -> Path:
    d = Path.home() / ".claude" / "skills" / "wp-cli" / "data"
...
def run_wp(site: dict, wp_args: list[str], json_format: bool = True,
           optional: bool = False) -> str | dict | list | None:
    """Run a WP-CLI command on the remote site via SSH.
    If optional=True, return None on failure instead of exiting."""
    ...
    # Redact sensitive flags from command echo (handles both --flag=value and --flag value)
...
def cmd_flush_cache(args):
    ...
    out = run_wp(site, ["rocket", "clean", "--confirm"], json_format=False, optional=True)
    if out is not None:
    ...
    # wp rocket preload
    out = run_wp(site, ["rocket", "preload"], json_format=False, optional=True)
    if out is not None:
