...
```bash
~/.claude/skills/sag/scripts/sag.sh "Hello world"
```

**Save to file (MP3/WAV/etc inferred from extension):**
```bash
~/.claude/skills/sag/scripts/sag.sh -o output.mp3 "Save this to a file"
```

**Choose a voice:**
```bash
~/.claude/skills/sag/scripts/sag.sh -v Roger "Hello in Roger's voice"
```

**List available voices:**
```bash
~/.claude/skills/sag/scripts/sag.sh -v ?
```

**Pipe input:**
```bash
echo "Piped text" | ~/.claude/skills/sag/scripts/sag.sh
```
...
```bash
# List all voices
~/.claude/skills/sag/scripts/sag.sh voices

# Search voices by language or name
~/.claude/skills/sag/scripts/sag.sh voices --search english
~/.claude/skills/sag/scripts/sag.sh voices --search english --limit 10
...
Example:
```bash
~/.claude/skills/sag/scripts/sag.sh "[whispers]This is a secret[/whispers] [short pause] Just kidding!"
```
...
```bash
# Basic TTS
~/.claude/skills/sag/scripts/sag.sh "Good morning!"

# Save narration to file
~/.claude/skills/sag/scripts/sag.sh -v Roger -o narration.mp3 "Once upon a time..."

# Fast model for quick responses
~/.claude/skills/sag/scripts/sag.sh --model-id eleven_flash_v2_5 "Quick response"

# Slow dramatic reading
~/.claude/skills/sag/scripts/sag.sh --speed 0.8 -v Roger "The night was dark and stormy..."
```
