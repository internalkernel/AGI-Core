...
def get_data_dir() -> Path:
    d = Path.home() / ".claude" / "skills" / "wp-management" / "data"
...
def cmd_themes_get(args):
    site = get_site(args.alias)
    data = wp_request(site, "GET", f"wp/v2/themes/{args.stylesheet}")
    print(json.dumps(data, indent=2))
...
def cmd_themes_activate(args):
    ...
    data = wp_request(site, "POST", f"wp/v2/themes/{args.stylesheet}", json_data={"status": "active"})
...
def cmd_seo_get(args):
    ...
    try:
        keywords = wp_request(site, "GET", f"seopress/v1/posts/{args.post_id}/target-keywords")
    except SystemExit:
        keywords = {}
    # Fetch robots settings
    try:
        robots = wp_request(site, "GET", f"seopress/v1/posts/{args.post_id}/meta-robot-settings")
    except SystemExit:
        robots = {}
