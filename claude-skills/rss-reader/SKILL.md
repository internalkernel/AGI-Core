---
name: rss-reader
description: Subscribe to RSS feeds and fetch new items with deduplication. JSON output for agent consumption.
---
...
```bash
RSS="uv run ~/.claude/skills/rss-reader/scripts/rss-reader.py"
```
...
Feed data is stored at `~/.claude/skills/rss-reader/data/`.
