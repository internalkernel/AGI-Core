...
def get_data_dir() -> Path:
    d = Path.home() / ".claude" / "skills" / "rss-reader" / "data"
...
def load_json(path: Path) -> dict | list:
    if path.exists():
        try:
            ...
            stderr(f"Warning: Corrupt JSON in {path}, resetting to empty")
            return {}
    return {}
...
def cmd_fetch(args):
    ...
    for url, info in target_feeds.items():
        ...
        # Preserve insertion order: keep existing entries first, append new ones
        prev_list = state.get(url, [])
        new_ids = new_seen - seen
        seen_list = prev_list + sorted(new_ids)
