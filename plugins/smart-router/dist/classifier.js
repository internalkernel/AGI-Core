...
export function classify(query, verbose = false) {
    // Calculate scores for each dimension
    const scores = {};
    let totalScore = 0;
    for (const [dimension, weight] of Object.entries(WEIGHTS)) {
        const rawScore = scoreDimension(query, dimension);
    ...
    const reasoningCount = PATTERNS.reasoningMarkers.filter(p => query.toLowerCase().includes(p)).length;
    let tier;
    let reasoning;
    if (reasoningCount >= 2) {
        tier = "REASONING";
...
export function getFallbackModel(tier) {
