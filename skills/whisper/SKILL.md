...
**Basic transcription:**
```bash
uv run ~/.openclaw/skills/whisper/scripts/transcribe.py audio.mp3
```
...
```bash
uv run ~/.openclaw/skills/whisper/scripts/transcribe.py foreign.wav \
...
```bash
uv run ~/.openclaw/skills/whisper/scripts/transcribe.py meeting.mp3 --format json --output meeting-transcript.json
```

**Generate subtitles:**
```bash
uv run ~/.openclaw/skills/whisper/scripts/transcribe.py video-audio.mp3 --format srt --output subtitles.srt --timestamps
```

**Quick transcription to stdout:**
```bash
uv run ~/.openclaw/skills/whisper/scripts/transcribe.py voicenote.m4a
```

**Translate Spanish audio:**
```bash
uv run ~/.openclaw/skills/whisper/scripts/transcribe.py spanish.mp3 --task translate --model medium
```
