---
name: aws-cli
description: AWS CLI wrapper for agents. Manage AWS infrastructure across EC2, S3, RDS, CloudFront, Route 53, Lambda, SES, ACM, WAF, Backup, IAM, and ElastiCache via subprocess with profile management and convenience wrappers. All output is JSON to stdout; status/errors to stderr.
---

# AWS CLI

Manage AWS infrastructure via the AWS CLI. Provides a passthrough for any AWS command plus convenience wrappers for 12 services: EC2, S3, RDS, CloudFront, Route 53, Lambda, SES, ACM, WAF, AWS Backup, IAM, and ElastiCache. All output is JSON to stdout; status/errors to stderr.
...
```bash
AWS="uv run ~/.openclaw/skills/aws-cli/scripts/aws-cli.py"
```
...
- **With workspace:** `$OPENCLAW_WORKSPACE/cache/aws-cli/`
- **Fallback:** `~/.openclaw/skills/aws-cli/data/`
