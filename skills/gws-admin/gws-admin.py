...
"""
OpenClaw Google Workspace Admin Skill (GAM7 Wrapper)
...
Usage:
    uv run gws-admin.py setup                          # First-time GAM install + config
    uv run gws-admin.py status                         # Check GAM install & auth status
    uv run gws-admin.py users list [--json]            # List all users
    uv run gws-admin.py users info <email>             # User details
    uv run gws-admin.py users create <email> --firstname X --lastname Y --password P
    uv run gws-admin.py users update <email> --suspended on/off
    uv run gws-admin.py users delete <email> --confirm # Delete user
    uv run gws-admin.py groups list [--json]           # List all groups
    uv run gws-admin.py groups info <email>            # Group details + members
    uv run gws-admin.py groups create <email> --name N
    uv run gws-admin.py groups add-member <group> <user>
    uv run gws-admin.py groups remove-member <group> <user>
    uv run gws-admin.py orgs list [--json]             # List org units
    uv run gws-admin.py orgs create <path>             # Create org unit
    uv run gws-admin.py orgs info <path>               # Org unit details
    uv run gws-admin.py aliases list [<user>]           # List aliases
    uv run gws-admin.py aliases create <alias> --target <user>
    uv run gws-admin.py aliases delete <alias> --confirm
    uv run gws-admin.py devices list [--json]          # Mobile devices
    uv run gws-admin.py devices wipe <id> --confirm    # Remote wipe
    uv run gws-admin.py licenses list [--json]         # License assignments
    uv run gws-admin.py raw <gam args...>              # Pass-through to raw GAM

Environment:
    GAMCFGDIR     — GAM config directory (default: ~/.openclaw/gam-config/)
...
GAM_CONFIG_DIR = Path.home() / ".openclaw" / "gam-config"
DESTRUCTIVE_OPS = {"delete", "wipe"}
...
def cmd_groups(args):
    ...
    if action == "list":
        ...
        if members_result.returncode == 0:
            if getattr(args, "json_output", False):
                print_json(gam_output_to_json(members_result.stdout))
            else:
                print("\n--- Members ---")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="gws-admin",
        description="OpenClaw Google Workspace Admin — GAM7 wrapper with safety guardrails",
