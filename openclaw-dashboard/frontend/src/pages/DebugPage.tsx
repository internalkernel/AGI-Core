...
export default function DebugPage() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [gw, h, s, l, fs] = await Promise.allSettled([
        api.fetchDebugGateway(activeAgent),
        api.fetchDebugHealth(activeAgent),
        api.fetchDebugSessions(activeAgent),
        api.fetchDebugLogs(activeAgent),
        api.fetchDebugFilesystem(activeAgent),
      ]);
  ...
  const agentLabel = AGENTS.find(a => a.id === activeAgent)?.name || activeAgent;
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex items-center justify-center h-64 text-slate-400">Loading diagnostics for {agentLabel}...</div>
      ) : (
        <>
          {/* Gateway Status */}
          <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-700/50">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-slate-500 ml-auto">{agentLabel}</span>
            </div>
          ...
          <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-700/50">
            ...
            {sessions.length === 0 ? (
              <EmptyState message={`No active sessions for ${agentLabel}`} />
            ) : (
